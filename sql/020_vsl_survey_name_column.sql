-- Follows sql/019. Same behaviour, updated vocabulary.
--
-- The import CSV's last column is now `name`, not `description` — the old
-- header never described what the value actually was, which is the feature's
-- name. The grouping column is likewise now a plain `id` rather than
-- `parcel_id`, since one identical file shape serves BLOCKS and PLOTS alike
-- and nothing should have to know which it is looking at.
--
-- Header renames happen in the edge function (normalizeCsvRow). What changes
-- here is only which p_items keys this RPC reads:
--
--   name       <- preferred, falls back to `descriptions`
--   feature_id <- preferred, falls back to `csv_parcel_id`
--
-- Both fallbacks are load-bearing during rollout: a browser running a cached
-- copy of the previous client still posts the old keys, and this must not
-- start silently naming every imported feature after its generated code.
--
-- Everything else — auto-numbered codes, per-item block_id from 017, estate
-- resolution — is carried over from 019 unchanged.
...
begin
  ...
  loop
    -- Grouping key, for error messages only. Never stored.
    v_ref := coalesce(v_item->>'feature_id', v_item->>'csv_parcel_id');
    begin
      v_geom := st_setsrid(st_geomfromgeojson((v_item->'geometry')::text), 4326);
      if v_geom is null or geometrytype(v_geom) <> 'POLYGON' then
        v_errors := v_errors || jsonb_build_array(jsonb_build_object('feature', v_ref, 'error', 'Invalid or non-polygon geometry'));
        continue;
      end if;

      -- The name, from the CSV's `name` column. Deliberately does NOT fall
      -- back to the id: that is a grouping key, not a name.
      v_name := nullif(trim(coalesce(v_item->>'name', v_item->>'descriptions', '')), '');
    ...
    exception
      when others then
        v_errors := v_errors || jsonb_build_array(jsonb_build_object('feature', v_ref, 'error', sqlerrm));
    end;
  end loop;
...
comment on function public.vsl_survey_batch_upsert(text, text, text, text, text, jsonb) is
  'Survey import commit, one path for BLOCKS and PARCELS. Codes are auto-numbered; names come from the CSV name column (p_items[].name); the CSV id is a grouping key only. p_items[].block_id overrides p_parent_block_code.';
