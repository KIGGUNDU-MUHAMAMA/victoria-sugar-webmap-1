# Sugarcane Estate Intelligence Platform — Full Feature Set & Schema (v4)

v4 change from v3: added Appendix A, the finalized activity-type list for the plot activity log / `plot_tasks.task_type` field, confirmed by you after reviewing the full ~70-item brainstorm. Everything else below is unchanged from v3.
...
**plot_tasks** — `task_id, plot_id, plot_season_id, block_id, task_type (see Appendix A for the finalized list), task_description, assigned_to, assigned_by, due_date, completed_date, status, estimated_cost, actual_cost, currency, notes, created_at`
...
## Appendix A — Finalized Activity Types (for `plot_tasks.task_type` / the map's "log activity" feature)

Reviewed down from the ~70-item brainstorm to these 18. This is the enum to build the activity-logging dropdown against:

- **Bush clearing** — clearing vegetation/undergrowth on new or fallow land before ploughing.
- **Ploughing** — primary tillage, turning over the soil.
- **Harrow** — secondary tillage, breaking clods after ploughing.
- **Ripping** — sub-soil ripping to break compacted soil layers.
- **Ridging** — forming raised beds/ridges ahead of planting.
- **Furrowing** — opening furrows/trenches for laying setts.
- **Lime application** — applying agricultural lime to correct soil acidity.
- **Planting** — laying sugarcane setts into furrows and covering them.
- **Manuring** — applying organic manure/compost.
- **Fertilization** — applying inorganic fertilizer (basal or top dressing).
- **Weeding** — manual or hand removal of weeds.
- **Cultivator** — mechanical inter-row cultivation to loosen soil and control weeds between rows.
- **Spraying** — pesticide/fungicide/herbicide application.
- **Irrigation** — watering the crop (furrow/drip/sprinkler).
- **Harvesting** — cutting and gathering mature cane.
- **Loading** — loading harvested cane onto trucks/trailers for transport.
- **Trash lining** — laying cut cane trash/residue in rows between planted rows after harvest.
- **Trash Collection** — gathering/removing trash from the field.

Note: `plot_tasks` already carries `estimated_cost`/`actual_cost` and `assigned_to` — pairing this enum with the "Number of labour" and "Current activity" fields from the plot detail panel spec means a single task record, once created via the map's log-activity button, can drive both the field-level popup and the cost/labor rollups.
